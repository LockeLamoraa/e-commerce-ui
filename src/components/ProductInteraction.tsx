...
export default function ProductInteraction({
    ...
    function handleTypeChange(type: string, value: string) {
    ...
    function handleQuantityChange(type: "increment" | "decrement") {
        setQuantity(prev => (type === "increment") ? prev + 1 : (quantity > 1) ? prev - 1 : 1);
    }
    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            ...
            <div className="flex flex-col gap-2 text-xs">
                ...
                <div className="flex items-center gap-2">
                    {product.colors.map(color => (
                        <div
                            className={`cursor-pointer border-1 p-[2px] ${selectedColor === color ? "border-gray-400" : "border-white"}`}
                            key={color}
            ...
            {/* BUTTONS */}
            <button
                className="flex items-center justify-center gap-2 bg-gray-800 text-white px-4 py-2 rounded-md shadow-lg cursor-pointer text-sm font-medium"
                onClick={handleAddToCart}
            >
                <Plus className="w-4 h-4" />
                Add to Cart
            </button>
            <button className="flex items-center justify-center gap-2 ring-1 ring-gray-400 text-gray-800 px-4 py-2 rounded-md shadow-lg cursor-pointer text-sm font-medium">
